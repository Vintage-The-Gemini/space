...
const NeoTracker = () => {
  ...
  const fetchNeoData = async () => {
    try {
      ...
      const response = await axios.get(
        `https://space-mgph.onrender.com/api/nasa/neo?start_date=${dateRange.start}&end_date=${dateRange.end}`
      );
  ...
  const prepareChartData = () => {
    ...
    return Object.entries(neoData.near_earth_objects).map(
      ([date, objects]) => ({
        date,
        total: objects.length,
        hazardous: objects.filter(
