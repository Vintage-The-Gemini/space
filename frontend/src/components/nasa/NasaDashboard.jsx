  ...
  useEffect(() => {
    const fetchNasaData = async () => {
      try {
        ...
        // Fetch APOD
        const apodResponse = await axios.get('http://localhost:5000/api/nasa/apod');
        setApodData(apodResponse.data);

        // Fetch Mars photos
        const marsResponse = await axios.get('http://localhost:5000/api/nasa/mars/curiosity');
        setMarsPhotos(marsResponse.data.photos.slice(0, 4));

        // Fetch NEO data
        const today = new Date().toISOString().split('T')[0];
        const neoResponse = await axios.get(`http://localhost:5000/api/nasa/neo?start_date=${today}&end_date=${today}`);
      ...
      {neoData && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-gray-800 p-6 rounded-lg">
            ...
            <p className="text-3xl text-white">
              {Object.values(neoData.near_earth_objects)[0]
          ...
          <div className="bg-gray-800 p-6 rounded-lg">
            ...
            {Object.values(neoData.near_earth_objects)[0].length > 0 && (
              <p className="text-3xl text-white">
                {Math.round(Object.values(neoData.near_earth_objects)[0][0]
                  .close_approach_data[0].miss_distance.lunar)}
                <span className="text-sm text-gray-400 ml-2">lunar distances</span>
              </p>
            )}
          </div>
        </div>
